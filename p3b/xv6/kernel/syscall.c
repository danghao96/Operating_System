...
{
  if((addr >= p->sz || addr+4 > p->sz) && (addr < p->stacksz))
    return -1;
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if(((uint)i >= proc->sz || (uint)i+size > proc->sz) && ((uint)i < proc->stacksz))
    return -1;
  if((uint)i >= USERTOP || (uint)i+size + 4 > USERTOP)
    return -1;
  if((uint)i < 2 * PGSIZE)
    return -1;
  *pp = (char*)i;
